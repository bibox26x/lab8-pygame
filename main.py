...
def grow_square(predator: Square, prey: Square) -> None:
    new_area = predator["size"] ** 2 + (prey["size"] ** 2) * PROPORTIONAL_AMOUNT
    new_size = int(new_area**0.5)
    predator["size"] = min(new_size, MAX_GROWTH_SIZE)

    speed = max(1, 6 - (predator["size"] // 10))
    predator["speed_x"] = speed if predator["speed_x"] >= 0 else -speed
    predator["speed_y"] = speed if predator["speed_y"] >= 0 else -speed
        ...
        for i, a in enumerate(squares):
            for b in squares[i + 1:]:
                ...
                if a["size"] > b["size"]:
                    grow_square(a, b)
                    respawn_square(b)
                    print(f"Square of size {a['size']} ate square of size {b['size']}")
                else:
                    grow_square(b, a)
                    respawn_square(a)
                    print(f"Square of size {b['size']} ate square of size {a['size']}")
